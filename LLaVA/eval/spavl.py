...
def main():
    ...
    for index in tqdm(range(len(eval_harm))):
        
        img_path = eval_harm[index]['image']
        question = eval_harm[index]['question']
        
        if img_path.mode != 'RGB':
            img_path = img_path.convert('RGB')
        file_path = f"../datasets/safety/spavl/images/{index}.jpg"
        img_path.save(file_path, format="JPEG")
        img_path = file_path
        ...
        with open(os.path.join(output_dir, f'spavl.json'), 'w') as f:
